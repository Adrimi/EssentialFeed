...
//
//  Created by adrian.szymanowski on 21/10/2020.
//
...
class EssentialFeedAPIEndToEndTests: XCTestCase {
    ...
    func test_endToEndTestServerGETFeedResult_matchesFixedTextAccountData() {
        ...
        case let .failure(error):
            XCTFail("Expected successful test feed result, got \(error) instead.")
        default:
            XCTFail("Expected successful test feed result, got nothing instead.")
        }
    }
    ...
    private func getFeedResult(file: StaticString = #file, line: UInt = #line) -> Swift.Result<[FeedImage], Swift.Error>? {
